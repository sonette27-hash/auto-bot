...
class EWrapperAccumulator(SignatureAccumulator):
    def getSignatures(self):
        for name, args in self.signatures:
            # Modern Python's `re` doesn't allow inline flags inside
            # a nested group like the original '(?!((?i)error.*))'.
            # Use a simple case-insensitive match to exclude names
            # that start with 'error'.
